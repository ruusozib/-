...
function setAnalyzing(isAnalyzing) {
  analyzingPanel.classList.toggle("hidden", !isAnalyzing);
}

function prepareResultForAnalyzing() {
  latencyChip.textContent = "処理時間: -- ms";
...
function rankCandidates(signature) {
  const scored = bagCatalog
    .map((item) => ({
      ...
      signatureDiff: {
        color:
          Math.round(
            ((Math.abs(signature.r - item.signature.r) +
              Math.abs(signature.g - item.signature.g) +
              Math.abs(signature.b - item.signature.b)) /
              765) *
              100
          ) / 100,
        edge: Math.round(Math.abs(signature.edge - item.signature.edge) * 100) / 100,
        contrast: Math.round(Math.abs(signature.contrast - item.signature.contrast) * 100) / 100,
      },
    }))
  ...
  return scored.map((item) => {
    ...
    return {
      ...
      confidence,
      evidence:
        item.signatureDiff.color < 0.15
          ? "色味が近い"
          : item.signatureDiff.edge < 0.12
            ? "質感パターンが近い"
            : "コントラスト傾向が近い",
    };
  });
  ...
  topThree.forEach((candidate, index) => {
    ...
    li.innerHTML = `
      ...
      <div class="candidate-main">
        <span class="candidate-name">${candidate.brand} / ${candidate.model}</span>
        <small class="candidate-evidence">根拠: ${candidate.evidence}（色差 ${candidate.signatureDiff.color}）</small>
      </div>
      <span class="candidate-score">${candidate.confidence}%</span>
    `;
...
function renderHistory(data = null) {
  ...
  history.forEach((item) => {
    ...
    li.innerHTML = `
      ...
      <small>信頼度: ${item.confidence}% / ${item.source === "camera" ? "カメラ" : "アップロード"} / ${item.timestamp}</small>
    `;
...
function processImageToResult(imageLike, sourceLabel = "camera") {
  ...
  const sourceWidth = imageLike.videoWidth || imageLike.naturalWidth || imageLike.width;
  const sourceHeight = imageLike.videoHeight || imageLike.naturalHeight || imageLike.height;
  const maxSide = 1280;
  const scale = sourceWidth > maxSide || sourceHeight > maxSide ? maxSide / Math.max(sourceWidth, sourceHeight) : 1;

  snapshotEl.width = Math.max(1, Math.round(sourceWidth * scale));
  snapshotEl.height = Math.max(1, Math.round(sourceHeight * scale));
...
function scanFromCamera() {
  if (!cameraEl.videoWidth || !cameraEl.videoHeight) {
    setStatus("カメラ準備中です。数秒待ってから「スキャンする」を押してください。");
    setStatusType("status-error");
    return;
  }
